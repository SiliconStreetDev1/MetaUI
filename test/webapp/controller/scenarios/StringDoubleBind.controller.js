sap.ui.define([
    ...
    return BaseController.extend("metaui.sandbox.controller.scenarios.StringDoubleBind", {
        onInit: function () {
            var oViewModel = this.setupViewModel();
            
            // Load the code example for this specific scenario
            var sSnippet = SnippetService.getCodeSnippet("string_double_bind");
            oViewModel.setProperty("/codeExamples", sSnippet);
            
            // Load the scenario description
            var sDesc = SnippetService.getScenarioDescription("string_double_bind");
            oViewModel.setProperty("/scenarioDescription", sDesc);

            MockDataService.loadScenario("string_double_bind")
