sap.ui.define([
    ...
    return BaseController.extend("metaui.sandbox.controller.scenarios.MixedLayout", {
        onInit: function () {
            var oViewModel = this.setupViewModel();
            
            // Load the code example for this specific scenario
            var sSnippet = SnippetService.getCodeSnippet("mixed_layout");
            oViewModel.setProperty("/codeExamples", sSnippet);
            
            // Load the scenario description
            var sDesc = SnippetService.getScenarioDescription("mixed_layout");
            oViewModel.setProperty("/scenarioDescription", sDesc);

            MockDataService.loadScenario("mixed_layout")
