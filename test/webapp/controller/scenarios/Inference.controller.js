sap.ui.define([
    ...
    return BaseController.extend("metaui.sandbox.controller.scenarios.Inference", {
        onInit: function () {
            var oViewModel = this.setupViewModel();
            
            // Load the code example for this specific scenario
            var sSnippet = SnippetService.getCodeSnippet("inference");
            oViewModel.setProperty("/codeExamples", sSnippet);
            
            // Load the scenario description
            var sDesc = SnippetService.getScenarioDescription("inference");
            oViewModel.setProperty("/scenarioDescription", sDesc);

            MockDataService.loadScenario("inference")
