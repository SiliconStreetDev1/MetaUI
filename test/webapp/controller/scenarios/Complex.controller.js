sap.ui.define([
    ...
    return BaseController.extend("metaui.sandbox.controller.scenarios.Complex", {
        onInit: function () {
            console.log("[MetaUI Sandbox] Complex controller initialized, loading mock data...");
            var oViewModel = this.setupViewModel();
            
            // Load the code example for this specific scenario
            var sSnippet = SnippetService.getCodeSnippet("complex");
            oViewModel.setProperty("/codeExamples", sSnippet);
            
            // Load the scenario description
            var sDesc = SnippetService.getScenarioDescription("complex");
            oViewModel.setProperty("/scenarioDescription", sDesc);

            MockDataService.loadScenario("complex")
                .then(function (result) {
                    console.log("[MetaUI Sandbox] Complex mock data loaded successfully.");
