sap.ui.define([
    ...
    return BaseController.extend("metaui.sandbox.controller.scenarios.Partial", {
        onInit: function () {
            var oViewModel = this.setupViewModel();
            
            // Load the code example for this specific scenario
            var sSnippet = SnippetService.getCodeSnippet("partial");
            oViewModel.setProperty("/codeExamples", sSnippet);
            
            // Load the scenario description
            var sDesc = SnippetService.getScenarioDescription("partial");
            oViewModel.setProperty("/scenarioDescription", sDesc);

            MockDataService.loadScenario("partial")
